...
public class Playground extends SurfaceView implements OnTouchListener{
	...
	private boolean isAtEdge(Dot d){
		if (d.getX()*d.getY()==0||d.getX()+1==COL||d.getY()+1==ROW) {//找错Bug发现在这里数组越界报错
			return true;
		}
		return false;
	}
...
//		System.out.println("X:"+one.getX()+" Y:"+one.getY()+" Dir:"+dir);
	...
	private void lose() {
		Toast.makeText(getContext(), "Obviously, You are a Loser!...", Toast.LENGTH_SHORT).show();
	}
	private void win() {
		Toast.makeText(getContext(), "Congratulations ,You are a Winner!...", Toast.LENGTH_SHORT).show();
	}
	private void redraw() {
